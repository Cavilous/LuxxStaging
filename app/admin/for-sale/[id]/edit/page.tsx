...
import { updateForSaleAsset } from "@/lib/for-sale-actions"
...
export default function EditForSaleAssetPage() {
  ...
  useEffect(() => {
    async function loadAsset() {
      setIsLoading(true)
      try {
        const response = await fetch(`/api/admin/for-sale/${id}`)
        const result = await response.json()
        
        if (!response.ok || result.error) {
          toast.error(result.error || "Failed to load asset")
        ...
        setFormData({
          ...
          advertisedPrice: asset.advertisedPrice || "",
          managedAssetPrice: asset.managedAssetPrice || "",
          specs: (asset.specifications as Record<string, any>) || {},
          managementTerms: {
            ownerUseAllotment: managementTerms.ownerUseAllotment || "",
            minHoldMonths: managementTerms.minHoldMonths || "",
            revSharePctOwner: managementTerms.revSharePctOwner || "",
            revSharePctLuxx: managementTerms.revSharePctLuxx || "",
            estAnnualGross: managementTerms.estAnnualGross || "",
            estAnnualNetToOwner: managementTerms.estAnnualNetToOwner || "",
          },
          status: asset.status || "Draft",
          badges: (asset.badges as string[]) || [],
          images: (asset.images as string[]) || [],
          imageSourceUrls: (asset.imageSourceUrls as ImageSourceUrl[]) || [],
          seo: {
            slug: asset.slug || "",
  ...
  const handleSubmit = async (status: string) => {
    ...
    startTransition(async () => {
      const result = await updateForSaleAsset(id, submitData)
      if (result.error) {
        toast.error(result.error)
      } else {
        toast.success(`Asset ${status === "Live" ? "published" : "saved"} successfully`)
        router.push("/admin/for-sale")
      }
    })
  }
