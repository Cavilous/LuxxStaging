...
async function fetchSmugMugImages(galleryUrl: string): Promise<string[]> {
  try {
    ...
    console.log(`  🖼️  Fetching images from: ${galleryUrl.substring(0, 60)}...`)
...
async function main() {
  console.log('🚀 Starting PRODUCTION car import & update...')
  console.log('⚠️  WARNING: Importing/updating PRODUCTION database\n')
  ...
  for (let i = 0; i < cars.length; i++) {
    ...
    try {
      ...
      console.log(`\n📦 Processing ${i + 1}/${cars.length}: ${car.title}`)

      if (existing.length > 0) {
        const existingCar = existing[0]
        const hasImages = existingCar.images && Array.isArray(existingCar.images) && existingCar.images.length > 0
        
        // Update if: has SmugMug URL but no images
        if (car.smugmugUrl && !hasImages) {
          console.log(`  🔄 Updating (missing images)...`)
          
          const images = await fetchSmugMugImages(car.smugmugUrl)
          ...
          if (images.length > 0) {
            await db
              ...
              .where(eq(inventory.id, existingCar.id))
          ...
          skipped++
          console.log(`  ⏭️  Skipped (already has ${existingCar.images.length} images)`)
        } else {
          skipped++
          console.log(`  ⏭️  Skipped (no SmugMug URL)`)
        }
      } else {
        // New car - import it
        console.log(`  ➕ Importing new car...`)
        ...
        imported++
        console.log(`  ✅ Imported with ${images.length} images`)
      }

      // Small delay to avoid rate limits
      await new Promise(resolve => setTimeout(resolve, 300))

      // Progress update every 25 cars
      if ((imported + updated) % 25 === 0 && (imported + updated) > 0) {
        console.log(`\n📊 Progress: ${imported} new + ${updated} updated = ${imported + updated} total processed`)
      }

    } catch (error) {
      console.error(`❌ Failed to process ${car.title}:`, error)
      failed++
    }
  }
  ...
  console.log('═══════════════════════════════════════')
  console.log(`➕ New cars imported:  ${imported}`)
  console.log(`🔄 Cars updated:       ${updated}`)
  console.log(`⏭️  Skipped:            ${skipped}`)
  console.log(`❌ Failed:             ${failed}`)
  console.log('═══════════════════════════════════════\n')
  
  console.log('🎉 Import complete! All new cars are set to DRAFT status.')
